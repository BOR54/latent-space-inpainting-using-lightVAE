...
class LiteAutoencoderKL(pl.LightningModule):
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        if bad_param or bad_grad:
            msg_lines = []
            msg_lines.append("===== NaN / Inf detected =====")
            msg_lines.append(f"global_step={self.global_step} batch_idx={batch_idx}")

            if bad_param:
                p = dict(self.named_parameters())[bad_param]
                msg_lines.append(f"BAD PARAM: {bad_param}")
                msg_lines.append(f"  data min={p.data.min().item()} max={p.data.max().item()} dtype={p.data.dtype} device={p.data.device}")

            if bad_grad:
                g = dict(self.named_parameters())[bad_grad].grad
                msg_lines.append(f"BAD GRAD: {bad_grad}")
                msg_lines.append(f"  grad min={g.min().item()} max={g.max().item()} dtype={g.dtype} device={g.device}")

            msg_lines.append("==============================")
            msg = "\n".join(msg_lines)

            # print and flush immediately (SLURM-safe)
            print(msg, flush=True)

            # also write to a file so we never lose it
            out_dir = getattr(self.trainer, "default_root_dir", ".")
            os.makedirs(out_dir, exist_ok=True)
            with open(os.path.join(out_dir, "nan_debug.txt"), "a") as f:
                f.write(msg + "\n")

            raise RuntimeError("NaN/Inf detected during training")
    ...
    def training_step(self, batch, batch_idx):
      ...
      # freeze the disc due to gradient accumulation (noop for our simple loss)
      self.hold_disc_grads(True)
  
      # get reconstructions
      reconstructions, posterior = self(inputs)
  
      # compute AE loss (we pass optimizer_idx=0 to keep your loss API consistent)
      ...
      # only log rec_loss if it exists (your SimpleVAELoss provides it)
      if 'train/rec_loss' in log_dict_ae:
